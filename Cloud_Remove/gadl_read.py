...
def gdalstore(img_path, name_path, endwith):
    ...
    datasetnew.WriteRaster(0, 0, im_width, im_height, im_data.tobytes(), im_width, im_height,
                           band_list=[3, 2, 1])  # 读入像素信息，波段排列为rgb
...
if __name__ == '__main__':
    path = r'E:\Desktop\DataSet_old\ALL\28\images\train'
    ...
    for i in tqdm.trange(len(img_files)):
        ...
        path_locate = path_name + endwith
        name = path_locate + '\\' + img.lstrip(path).rstrip('.tif')
        #  获取正确路径方便调用
