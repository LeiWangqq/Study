...
class MoCo_v2(nn.Module):
    ...
    def __init__(self,
        ...
        # create the queue
        # register_buffer通常被用在这么一种情况：我们需要保存一个状态，但是这个状态不能看作成为模型参数
    ...
    def moco_forward(self, q, k, prints=False):
        ...
        q_enc = self.q_encoder(q)  # queries: NxC  (batches,2048)
        q = self.q_fc(q_enc)       # (batches,128)
        ...
        # positive logits: Nx1   [batch,1]
        # 采用爱因斯坦求和约定
        l_pos = torch.einsum('nc,nc->n', [q, k]).unsqueeze(-1)
        print('l_pos', l_pos.shape) if prints else None

        # negative logits: NxK   [batch,16384]
        ...
        self.update_k_encoder_weights()  # 根性使用q_encoder更新k_encoder
        self.update_queue(k)   # 更行队列
